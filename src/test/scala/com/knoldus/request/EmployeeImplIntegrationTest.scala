...
class EmployeeImplIntegrationTest extends AnyFunSuite {
  ...
  test("User not created because email is invalid"){
    val Neha: Employee = new Employee("Neha", "Poddar", 22, 12000, "Intern", "Knoldus", "Neha.Kumari@gmailcom")
    val result = employeeImpl.createEmployee(Neha)
    assert(result.isEmpty)
  }
  ...
  test("User can be created"){
    val Neha: Employee = new Employee("Neha","Poddar", 22, 12000, "Intern", "Knoldus", "Neha.kuamri@gmail.com")
    val result = employeeImpl.createEmployee(Neha)
